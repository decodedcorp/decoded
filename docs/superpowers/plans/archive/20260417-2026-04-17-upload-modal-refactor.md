...
vi.mock("gsap", () => ({ gsap: { to: vi.fn(), set: vi.fn() }, default: { to: vi.fn(), set: vi.fn() } }));
vi.mock("@/lib/stores/requestStore", () => ({
  useRequestStore: Object.assign(
    () => ({ resetRequestFlow: vi.fn() }),
...
describe("RequestFlowModal — maxWidth prop", () => {
  ...
  test("defaults to max-w-4xl when maxWidth not supplied", () => {
    ...
    const dialog = screen.getByTestId("child").closest("[data-testid='request-flow-modal-dialog']");
...
describe("RequestFlowModal — onClose prop", () => {
  test("calls onClose when backdrop is clicked", () => {
    ...
    const backdrop = container.querySelector("[data-testid='request-flow-modal-backdrop']");
...
describe("requestStore — activeInstanceId guard", () => {
  ...
  test("resetIfActive resets only when instance matches", () => {
    useRequestStore.getState().setActiveInstance("inst-1");
    useRequestStore.setState({ image: { url: "data:x", file: null as any, isCompressed: false } as any });
...
describe("useUploadFlow — submit", () => {
  ...
  test("submit clears draft and resets active instance on success", async () => {
    useRequestStore.setState({
      image: { url: "data:x", file: new File(["x"], "x.jpg"), isCompressed: false } as any,
...
import { createPostWithFile, createPostWithFileAndSolutions } from "@/lib/api/posts";
...
const submit = useCallback(async () => {
  ...
  try {
    ...
    const file = s.image.isCompressed ? s.image.file : await compressImage(s.image.file);
...
describe("UploadFlowSteps — step branches", () => {
  ...
  test("renders userKnowsItems fork after image upload", () => {
    useRequestStore.setState({
      image: { url: "data:x", file: new File(["x"], "x.jpg"), isCompressed: true } as any,
    });
    render(<UploadFlowSteps />);
...
export function UploadFlowSteps() {
  ...
  if (!image) {
    return <div data-testid="upload-flow-dropzone"><DropZone /></div>;
  }
  ...
  return (
    <div data-testid="upload-flow-active">
      ...
      <button disabled={flow.isSubmitting} onClick={flow.submit}>Post</button>
    </div>
  );
}
```
...
test.describe("Upload intercept modal", () => {
  test("in-app nav opens modal, background preserved, upload succeeds", async ({ page }) => {
    await page.goto("/");
    ...
    await expect(page.getByTestId("request-flow-modal-dialog")).not.toBeVisible();
  });
});
```
...
vi.mock("@/lib/stores/requestStore", () => ({
  useRequestStore: Object.assign(
    (sel: any) => sel({
      metadata: { mediaType: "photoshoot", artist: "", group: "", platform: "", year: "" },
      setMetadata: vi.fn(),
    }),
