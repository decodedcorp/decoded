...
## Current Milestone: v8.0 Monorepo Consolidation & Bun Migration — SHIPPED
...
- ✓ Yarn 4 → bun 1.3.10 패키지 매니저 전환 (workspaces, lock file, scripts)
- ✓ Turborepo 빌드 오케스트레이션 도입
- ✓ TypeScript 빌드 에러 수정 (Fragment 래핑 + Supabase 타입 캐스팅)
- ✓ middleware.ts → proxy.ts (Next.js 16 convention)
- ✓ README.md, CLAUDE.md 모노레포 구조 반영
- ✓ GitHub 레포 생성 (decodedcorp/decoded-monorepo)

**Remaining (CI/CD — deferred to v8.1):**
...
**v8.0 Monorepo Consolidation & Bun Migration (shipped 2026-03-23):**

- ✓ Backend subtree 병합 (packages/backend/, git history 보존)
- ✓ Yarn 4 → bun 1.3.10 패키지 매니저 전환
- ✓ Turborepo 빌드 오케스트레이션 도입
- ✓ 통합 개발환경 구성 (`bun run dev` via Turborepo)
- [ ] CI/CD 워크플로우 통합 (deferred to v8.1)
...
- **백엔드**: packages/backend/ (Rust/Axum, subtree merged from decodedcorp/backend)
...
_Last updated: 2026-03-23 after v8.0 milestone shipped_
