...
## Current Milestone: v9.0 Type-Safe API Generation

**Goal:** Orval + Zod를 사용하여 백엔드 OpenAPI spec에서 타입 안전한 API 클라이언트를 자동 생성하고, 기존 수동 API 호출 코드를 교체

**Target features:**
- Orval + Zod 코드 생성 파이프라인 구축
- 전체 백엔드 API 엔드포인트 클라이언트 자동 생성
- 기존 수동 API 호출 코드(lib/api/, React Query hooks)를 생성 코드로 마이그레이션
...
**v9.0 Type-Safe API Generation:**

- [x] Orval + Zod 코드 생성 파이프라인 구축 — Phase 39-40 완료
- [x] 백엔드 OpenAPI spec 기반 타입/클라이언트 자동 생성 — Phase 40 완료
- [x] 기존 수동 API 호출 코드를 생성 코드로 교체 — Phase 41: GET 마이그레이션, Phase 42: mutation 마이그레이션 + 수동 API 파일 삭제 완료
- [x] CI hardening + tooling — Phase 43: pre-push drift 감지, .gitignore, Zod 검증 테스트, 에이전트 문서화 완료
...
_Last updated: 2026-03-24 after Phase 43 (CI Hardening and Tooling) complete — v9.0 마일스톤 최종 phase, pre-push drift 감지, Zod 검증 테스트, 에이전트 문서화 완료_
