...
**v9.0 (2026-03-23 shipped):** Type-Safe API Generation — Orval + Zod 코드 생성, 수동 API 코드 전량 교체, CI drift 감지

## Current Milestone: v10.0 Tech Debt Resolution

**Goal:** 프로덕션 안정성과 코드 품질을 위한 기술 부채 해소 — Rate Limiting, 에러 트래킹, 테스트 커버리지, 컴포넌트 리팩토링, 메모리 누수 수정

**Target features:**
- Rate Limiting 적용 (AI API 비용 보호) — #11
- Sentry 에러 트래킹 도입 (프론트엔드 + 백엔드 + AI 서버) — #12
- 핵심 비즈니스 로직 E2E 테스트 작성 — #13
- 대형 컴포넌트 분리 (ThiingsGrid, ImageDetailContent 등) — #14
- 메모리 누수 방지 (ObjectURL, 이벤트 리스너 정리) — #15
...
**v10.0 Tech Debt Resolution:**

- [ ] Rate Limiting 적용 — AI API 비용 보호, Axum 미들웨어 + Next.js proxy 레벨
- [ ] Sentry 에러 트래킹 도입 — 프론트엔드/백엔드/AI 서버 런타임 모니터링
- [ ] 핵심 비즈니스 로직 E2E 테스트 — Playwright 기반, 핵심 플로우 커버리지
- [ ] 대형 컴포넌트 분리 — ThiingsGrid, ImageDetailContent 등 600줄+ 컴포넌트 리팩토링
- [ ] 메모리 누수 방지 — ObjectURL 해제, 이벤트 리스너 클린업
...
_Last updated: 2026-03-26 after v10.0 Tech Debt Resolution milestone initialization_
