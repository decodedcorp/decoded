...
## Current Milestone: v8.0 Monorepo Consolidation & Bun Migration

**Goal:** Backend 레포(Rust/Axum)를 모노레포에 subtree 병합하고, Yarn 4 → bun 전환 + Turborepo 도입으로 통합 개발환경 구성

**Target features:**

- Backend Rust 코드를 `packages/backend/`로 subtree 병합 (git history 보존)
- Yarn 4 → bun 패키지 매니저 전환 (workspaces, lock file, scripts)
- Turborepo 빌드 오케스트레이션 도입
- 통합 개발환경 (`bun dev` → 프론트+백 동시 기동)
- 환경변수 및 Docker 구성 통합
- CI/CD 워크플로우 통합 (path-based change detection)
...
- [ ] Backend subtree 병합 (packages/backend/, git history 보존)
- [ ] Yarn 4 → bun 패키지 매니저 전환
- [ ] Turborepo 빌드 오케스트레이션 도입
- [ ] 통합 개발환경 구성 (프론트+백 동시 기동)
- [ ] 환경변수 및 Docker 구성 통합
- [ ] CI/CD 워크플로우 통합
...
- **백엔드 레포**: https://github.com/decodedcorp/backend (26,826 LOC, 19 domains, 44 migrations)
- **백엔드 API**: https://dev.decoded.style (OpenAPI spec 완비)
- **패키지 매니저**: Yarn 4.9.2 → bun 1.3.10 전환 예정
...
| Monorepo merge via git subtree                     | Git history 보존하면서 단일 레포로 통합            | — Pending  |
| Yarn 4 → bun migration                             | 더 빠른 설치/빌드, 통합 런타임                     | — Pending  |
| Turborepo for build orchestration                  | 프론트/백 독립 빌드 + 캐싱, CI 효율화              | — Pending  |
| Backend stays outside Yarn workspace               | Cargo workspace는 독립 — bun은 JS 패키지만 관리    | — Pending  |

---

_Last updated: 2026-03-22 after v8.0 milestone start_
