...
from src.config.helpers import use_prod_mock_overrides
from src.config import Environment


@pytest.fixture(scope="module")
def container():
    environment = Environment.from_environ(env_file=".prod.env")
    container = Application(environment=environment)
    use_prod_mock_overrides(container)
...
def lmstudio_manager(container):
    return container.managers.lmstudio_manager()
...
def test_lmstudio_manager(lmstudio_manager):
    assert lmstudio_manager is not None
    assert lmstudio_manager.health_check()
