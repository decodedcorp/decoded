...
class TestConcurrentProcessing:
    ...
    async def test_concurrency_limits_respected(
        self, batch_processor_low_concurrency, mock_link_processor, mock_image_processor
    ):
    ...
    async def test_performance_scaling_with_concurrency(
        self, mock_link_processor, mock_image_processor
    ):
    ...
    async def test_memory_usage_with_large_batches(
        self, batch_processor_high_concurrency, mock_link_processor, mock_image_processor
    ):
        ...
        mock_image_processor.process_image.return_value = ImageResult(
            url="https://example.com/test.jpg", processing_status=ProcessingStatus.SUCCESS
        )
        ...
        for batch_size in [100, 500, 1000]:
            ...
            result = await batch_processor_high_concurrency.process_batch(
                f"memory-test-{batch_size}", items
            )
    ...
    async def test_error_handling_performance_impact(
        ...
        assert total_time < expected_time * 1.5, (
            f"Error handling caused significant slowdown: {total_time:.2f}s vs expected ~{expected_time:.2f}s"
        )

        # Verify mixed results
        total_results = len(result.link_results) + len(result.image_results)
        assert total_results == 50
        assert result.statistics.failed_count == len(
            [item for item in items if item.item_id in error_items]
        )

    @pytest.mark.asyncio
    async def test_semaphore_contention_performance(
        self, mock_link_processor, mock_image_processor
    ):
    ...
    @pytest.mark.asyncio
    async def test_batch_size_performance_characteristics(
        self, batch_processor_high_concurrency, mock_link_processor, mock_image_processor
    ):
    ...
    async def test_resource_cleanup_performance(
        ...
        assert final_task_count <= initial_task_count + 2, (
            f"Tasks not cleaned up: {initial_task_count} -> {final_task_count}"
        )

    @pytest.mark.asyncio
    async def test_performance_under_mixed_workload(
        self, batch_processor_high_concurrency, mock_link_processor, mock_image_processor
    ):
        ...
        max_expected_time = 0.8 * (
            50 / batch_processor_high_concurrency.max_concurrency
        )  # Worst case scenario
