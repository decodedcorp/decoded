...
class MetadataContainer(DeclarativeContainer):
    ...
    searxng_client: Singleton[SearXNGClient] = Singleton(
        SearXNGClient,
        environment=environment
    )
    ...
    og_extractor: Singleton[OGTagExtractor] = Singleton(
        OGTagExtractor
    )
    ...
    content_resolver: Singleton[ContentTypeResolver] = Singleton(
        ContentTypeResolver
    )
    ...
    llm_router: Singleton[LLMRouter] = Singleton(
        LLMRouter,
        client_mapping={
            "link": [perplexity_client, groq_client, local_llm_client],  # Perplexity client (SDK-based) as first option
