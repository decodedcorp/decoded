...
async def extract_metadata(
    ...
    try:
        ...
        result = await manager._analyze_link(
            url=request.url, item_id=item_id, og_metadata=og_metadata
        )
        ...
        processing_time = time.time() - start_time
        raise HTTPException(
            status_code=500, detail=f"메타데이터 추출 중 오류가 발생했습니다: {str(e)}"
        )


@router.get("/extract-og", response_model=ExtractOGResponse)
async def extract_og_metadata(
    url: str, manager: Annotated[MetadataExtractService, Depends(get_metadata_extract_service)]
) -> ExtractOGResponse:
    """
