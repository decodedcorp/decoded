...
class GroqClient(BaseLLMClient):
    ...
    def _truncate_content(self, content: str, max_tokens: int = 6000) -> str:
        ...
        self.logger.warning(
            f"Content truncated from {estimated_tokens} to ~{self._estimate_tokens(truncated)} tokens"
        )
    ...
    async def _make_api_request(
        ...
        if estimated_tokens > 7500:  # Leave buffer for response tokens
            self.logger.warning(
                f"High token usage detected: ~{estimated_tokens} tokens. This might exceed Groq's limit."
            )
    ...
    def _parse_link_response(self, response: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if start_idx >= 0 and end_idx > start_idx:
                ...
                return {"description": content[:300], "usage": response.get("usage", {})}
    ...
    async def _handle_link_completion(self, messages: List[LLMMessage], **kwargs) -> LLMResponse:
        ...
        system_message = (
            "You are a professional content analyzer. Provide accurate and concise analysis."
        )
    ...
    async def _handle_text_completion(
        ...
        **kwargs,
        ...
        return LLMResponse(provider="groq", content=response.get("content", ""), usage=usage)
