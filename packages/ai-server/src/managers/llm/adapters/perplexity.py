...
class PerplexityClient(BaseLLMClient):
    ...
    def _normalize_messages(self, messages: List[LLMMessage]) -> List[Dict[str, str]]:
        ...
        if current_role and current_content_parts:
            normalized.append({"role": current_role, "content": "\n".join(current_content_parts)})
    ...
    async def _handle_text_completion(
        self, messages: List[LLMMessage], response_schema: Optional[Type[BaseModel]] = None
    ) -> LLMResponse:
        """
    ...
    async def _handle_image_completion(
        ...
        **kwargs,
        ...
        try:
            ...
            system_message = (
                "You are professional image analyzer. All of the info must be your own analysis"
            )
            ...
            if structured_output.get("objects"):
                objects_str = (
                    ", ".join(structured_output["objects"])
                    if isinstance(structured_output["objects"], list)
                    else structured_output["objects"]
                )
