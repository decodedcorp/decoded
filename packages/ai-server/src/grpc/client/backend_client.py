...
from src.database.models.content import ProcessedLinkMetadata, ProcessedImageMetadata, QAPair
...
class GRPCBackendClient:
    ...
    async def connect(self, timeout: float = 30.0):
        ...
        """
        try:
            self.logger.info(f"Attempting to connect to backend server at {self.address} (timeout: {timeout}s)")
    ...
    def _build_processed_link_metadata(self, link_result: Dict[str, Any]) -> Optional[ProcessedLinkMetadata]:
        """
