...
class MetadataServicer(inbound_pb2_grpc.QueueServicer):
    ...
    async def AnalyzeLinkDirect(
        ...
        try:
            url = request.url
            solution_id = request.post_id  # Note: proto field is still 'post_id' but represents solution_id
            title = request.title
            ...
            if result.status == ProcessingStatus.SUCCESS and result.metadata:
                ...
                if metadata.qa_pairs:
                    for qa in metadata.qa_pairs:
                        qna_list.append(inbound_pb2.QAPair(
    ...
    async def AnalyzeLink(
        ...
        try:
            ...
            if success:
                self.logger.debug(f"AI analysis request enqueued for solution_id: {solution_id} (job_id: {job_id})")
            ...
            return inbound_pb2.AnalyzeLinkResponse(
                success=False,
                message=f"Failed to enqueue analysis request: {str(e)}",
                batch_id=""
            )
    ...
    async def ProcessPostEditorial(
        ...
        try:
            ...
            self.logger.debug(f"Received ProcessPostEditorial request for magazine {post_magazine_id}")
            ...
            if success:
                self.logger.debug(f"Post editorial enqueued for {post_magazine_id} (job_id: {job_id})")
