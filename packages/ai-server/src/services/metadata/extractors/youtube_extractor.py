...
class YouTubeExtractor(BaseExtractor):
    ...
    def __init__(self, og_extractor=None):
        ...
        if not YOUTUBE_TRANSCRIPT_AVAILABLE:
            self.logger.warning("youtube-transcript-api not available. Transcript extraction will be disabled.")
    ...
    async def extract_metadata(self, url: str) -> Dict[str, Any]:
        ...
        if not self.can_handle(url):
            return {
                'success': False,
                'error': f"URL is not a YouTube URL: {url}"
            }
        ...
        if YOUTUBE_TRANSCRIPT_AVAILABLE:
            ...
            if transcript_result['success']:
                ...
                self.logger.warning(f"Failed to get transcript for {video_id}: {transcript_result.get('error', 'Unknown error')}")
        else:
            self.logger.warning("youtube-transcript-api not available, skipping transcript extraction")
    ...
    async def _get_youtube_transcript(self, video_id: str, preferred_language: str = 'ko') -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if len(full_text) > max_transcript_length:
                ...
                self.logger.info(f"Truncated transcript for video {video_id}: original length {len(' '.join([item.text for item in transcript_data.snippets]))} chars -> {len(full_text)} chars")
