...
class SearXNGClient:
        ...
        try:
            ...
            if parsed.hostname:
                ...
                for part in hostname_parts[:-1]:  # Skip TLD
                    if part not in ['www', 'blog', 'news', 'api', 'cdn', 'static', 'images', 'assets']:
    ...
    def _is_meaningful_keyword(self, word: str) -> bool:
        ...
        common_words = {
            ...
            'index', 'home', 'main', 'default', 'content', 'detail',
            'view', 'show', 'list', 'item', 'items', 'category',
            'tag', 'tags', 'search', 'results', 'archive'
        }
    ...
    async def search_parallel(self, url: str, link_count: int = 3, image_count: int = 1) -> Dict[str, List[Dict[str, Any]]]:
        """
        ...
        try:
            ...
            return {
                ...
                'images': filtered_images[:image_count]
            }
    ...
    async def _perform_search(self, query: str, count: int) -> List[Dict[str, Any]]:
        ...
        for attempt in range(self.environment.SEARXNG_MAX_RETRIES):
            try:
                async with httpx.AsyncClient(timeout=self.environment.SEARXNG_REQUEST_TIMEOUT) as client:
    ...
    def _is_valid_image_result(self, result: Dict[str, Any]) -> bool:
        ...
        skip_patterns = [
            'placeholder', 'blank', 'spacer', 'pixel.gif', 'dot.gif',
            'loading', 'spinner', 'default', 'no-image', 'missing'
        ]
    ...
    def _calculate_quality_score(self, result: Dict[str, Any]) -> float:
        ...
        try:
            ...
            good_domains = [
                ...
                'googleusercontent.com', 'wikimedia.org',
    ...
    def _filter_and_rank_results_with_domain_matching(self, results: List[Dict[str, Any]], original_url: str) -> List[Dict[str, Any]]:
    ...
    async def _perform_link_search(self, query: str, count: int) -> List[Dict[str, Any]]:
        ...
        for attempt in range(self.environment.SEARXNG_MAX_RETRIES):
            try:
                ...
                self.logger.warning(f"Link search request failed on attempt {attempt + 1}: {str(e)}")
                if attempt < self.environment.SEARXNG_MAX_RETRIES - 1:
                    wait_time = (2 ** attempt) + 1
                    await asyncio.sleep(wait_time)
                    continue
    ...
    def _filter_and_rank_link_results(self, results: List[Dict[str, Any]], count: int) -> List[Dict[str, Any]]:
    ...
    def _is_valid_link_result(self, result: Dict[str, Any]) -> bool:
        ...
        spam_domains = [
            'spam', 'ads', 'advertisement', 'popup', 'redirect',
    ...
    def _calculate_link_quality_score(self, result: Dict[str, Any]) -> float:
        ...
        reputable_domains = [
            ...
            'medium.com', 'reddit.com', 'news.ycombinator.com',
