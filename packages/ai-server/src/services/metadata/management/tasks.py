...
async def retry_failed_items(
    ...
    try:
        ...
        logger.info(
            f"Retry task completed: {success_count} succeeded, "
            f"{failed_count + error_count} failed"
        )
...
async def get_failed_items_stats(
    ...
    try:
        ...
        result = {
            "statistics": stats,
            "message": "Successfully retrieved failed items statistics"
        }
    ...
    async with semaphore:
        try:
            ...
            processed_batch = await metadata_extract_service.process_batch(
                batch_id=f"retry_{retry_item.item_id}_{int(time.time())}",
                items=[data_item]
            )
            ...
            if results and len(results) > 0:
                ...
                await unified_retry_manager.mark_retry_failed(
                    retry_item.item_id,
                    "No results returned from retry processing"
                )
    ...
    try:
        ...
        for retry_item in partial_items:
            try:
                ...
                result = await metadata_extract_service.process_partial_retries(1)  # Process one at a time
                if result.get('successful_count', 0) > 0:
                    successful_count += 1
                    await unified_retry_manager.complete_partial_processing(retry_item.item_id, True)
                else:
