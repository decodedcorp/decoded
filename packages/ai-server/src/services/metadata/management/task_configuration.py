...
def configure_metadata_tasks(
    ...
    cleanup_task = partial(
        tasks.cleanup_old_failed_items, unified_retry_manager=unified_retry_manager, max_age_days=7
    )
    scheduler.register_task("cleanup_old_items", cleanup_task, 21600)

    # Register stats task (every 5 minutes)
    stats_task = partial(tasks.get_failed_items_stats, unified_retry_manager=unified_retry_manager)
    ...
    partial_stats_task = partial(
        tasks.get_partial_retry_stats, unified_retry_manager=unified_retry_manager
    )
