...
from src.database.models.content import (
    LinkPreviewMetadata, ProcessedLinkMetadata, LinkProcessingResult, ProcessingStatus
)
...
from .schemas import (
    ProductResponse, ArticleResponse, 
    VideoResponse, OtherResponse
)
    ...
    async def _extract_with_llm(
        ...
        try:
            ...
            if link_preview.description:
                messages.append(LLMMessage(role="user", content=f"Description: {link_preview.description}"))
            messages.append(LLMMessage(role="user", content=f"Platform: {host}"))
            messages.append(LLMMessage(role="user", content="이 링크의 내용을 1-2문장으로 요약해서 summary 필드에 넣어줘"))
            messages.append(LLMMessage(role="user", content="추가로 이 링크에 대한 질문과 답변 3-5개를 qna 필드에 포함해줘"))
            messages.append(LLMMessage(role="user", content="키워드는 일본어, 태국어, 한국어, 영어 순서대로 뽑아줘"))
            messages.append(LLMMessage(role="user", content="중요: metadata 필드의 모든 값은 반드시 영어로 작성해줘"))
