...
from src.database.models.content import (
    ProcessedImageMetadata, ImageProcessingResult, ProcessingStatus
)
...
class ImageProcessor:
    ...
    async def process_image(self, image_url: str, item_id: str) -> ImageProcessingResult:
        ...
        try:
            ...
            messages = [LLMMessage(role="user", content="Analyze this image and provide detailed metadata")]
            ...
            analysis_result = llm_response.structured_output if llm_response.structured_output else {}
            ...
            self.logger.info(
                f"Successfully processed image {item_id} in {processing_time:.2f}s: "
                f"status={status}"
            )
            ...
            return ImageProcessingResult(
                ...
                url=image_url,
                status=status,
                metadata=processed_image_metadata
            )
    ...
    async def process_image_data(self, image_data: bytes, item_id: str, category_rules: Optional[Dict[str, list]] = None) -> ImageProcessingResult:
        """
        ...
        try:
            ...
            # Phase 2: Compress image if needed
            compressed_image_data = self.image_compressor.compress_if_needed(image_data)
            self.logger.info(f"Binary image size after compression: {len(compressed_image_data)} bytes")
    ...
    async def _save_image_to_temp_file(self, image_data: bytes, item_id: str) -> str:
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(delete=False, suffix='.jpg', prefix=f'img_{item_id}_') as temp_file:
