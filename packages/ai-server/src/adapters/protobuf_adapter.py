...
class ProtobufAdapter:
    ...
    def to_link_metadata(metadata: ProcessedLinkMetadata) -> outbound_pb2.LinkMetadata:
        ...
        for qa in metadata.qa_pairs:
            qa_pairs.append(outbound_pb2.QAPair(question=qa.question, answer=qa.answer))
    ...
    def to_batch_item_result(
        ...
        batch_item = outbound_pb2.BatchItemResult(
            item_id=item_id, url=url, type=item_type, status=status
        )
        ...
        if link_metadata:
            batch_item.link_metadata.CopyFrom(ProtobufAdapter.to_link_metadata(link_metadata))

        if image_metadata:
            batch_item.image_metadata.CopyFrom(ProtobufAdapter.to_image_metadata(image_metadata))
