 ...
 *
 * @throws Error if DATABASE_SERVICE_ROLE_KEY or NEXT_PUBLIC_DATABASE_API_URL is missing.
 */
export function createAdminSupabaseClient() {
  const supabaseUrl = getEnv("NEXT_PUBLIC_DATABASE_API_URL");
  const serviceRoleKey = process.env.DATABASE_SERVICE_ROLE_KEY;
  if (!serviceRoleKey) {
    throw new Error(
      "DATABASE_SERVICE_ROLE_KEY is not set. Admin mutations require service_role."
    );
  }
