...
## 변경 요약 (총 11개 파일: 4 rules + 7 skills)
...
### Phase 3: 생성기 스킬 — SpecKit 연동 (프로젝트 스킬)
...
### Phase 4: 로컬 스킬 포팅 — 범용 품질 도구 (프로젝트 스킬)

| 파일 | 소스 | 설명 |
|------|------|------|
| `.cursor/skills/security-auditor/SKILL.md` | `~/.claude/skills/security-auditor/SKILL.md` | OWASP Top 10 + 보안 취약점 스캔 |
| `.cursor/skills/testing-specialist/SKILL.md` | `~/.claude/skills/testing-specialist/SKILL.md` | AAA 패턴 테스트 케이스 작성 가이드 |
| `.cursor/skills/react-best-practices/SKILL.md` | `~/.claude/skills/vercel-react-best-practices/SKILL.md` | 45개 React/Next.js 성능 최적화 규칙 |
...
| data-model-generator | api-contract와 중복 |
| 에이전트 (performance-auditor 등) | 서브에이전트 패턴 Cursor 미지원 |
| pipeline | 멀티에이전트 오케스트레이션 Cursor 미지원 |
| doc-ecosystem-audit | Obsidian 볼트 의존, 개인용 |
| merge-worktree | EnterWorktree 도구 Claude Code 전용 |
...
└── skills/
    ...
    ├── code-reviewer/SKILL.md    # NEW (from .claude/agents/)
    ├── git-workflow/SKILL.md     # NEW (from .claude/skills/)
    ├── component-template/SKILL.md # NEW (from .claude/skills/)
    ├── api-contract/SKILL.md     # NEW (from .claude/skills/)
    ├── security-auditor/SKILL.md # NEW (from ~/.claude/skills/)
    ├── testing-specialist/SKILL.md # NEW (from ~/.claude/skills/)
    └── react-best-practices/SKILL.md # NEW (from ~/.claude/skills/)
```
