...
## Step 0: Orval/Zod generated 코드 최신화

1. Rust 백엔드에서 최신 `openapi.json` 생성 (`cargo run` 또는 기존 파일 확인)
2. `packages/api-server/openapi.json` → 최신 상태 확인
3. `cd packages/web && bun run generate:api` 실행
4. generated 코드 diff 확인 — 새로운 엔드포인트/타입 반영
...
## 커밋 전략
1. `chore(api): regenerate Orval/Zod from latest openapi.json`
2. `feat(api): add server-instance.ts for RSC API calls`
3. `refactor(main): migrate magazine/grid/dome to REST API with Orval types`
4. `chore(main): clean up unused Supabase query functions`
