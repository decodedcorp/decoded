...
const rx = r.w * 0.55 + rand() * 3; // ← 아이템 너비의 55% (살짝 여유)
const ry = r.h * 0.55 * aspectRatio + rand() * 3; // ← 높이 기반
```
...
let startX = r.cx, startY = r.cy, bestDist = Infinity;
for (const [cx, cy] of corners) {
  const d = Math.hypot(cx - targetX, cy - targetY);
  if (d < bestDist) { startX = cx; startY = cy; bestDist = d; }
}
```
